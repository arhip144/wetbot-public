    ...
    run: async (client, interaction, args) => {
        ...
        if (interaction.isChatInputCommand() || interaction.customId?.includes("view")) {
            ...
            if (args?.Subcommand === "delete") {
                achievement = client.cache.achievements.find(e => e.name.toLowerCase() === args.name.toLowerCase() && e.guildID === interaction.guildId)
                if (!achievement) return interaction.editReply({ content: `${client.config.emojis.NO}**${client.language({ textId: `Достижение с названием`, guildId: interaction.guildId, locale: interaction.locale })} <${args.name}> ${client.language({ textId: `не существует`, guildId: interaction.guildId, locale: interaction.locale })}**` })
                await achievement.delete()
                return interaction.editReply({ content: `${client.config.emojis.YES}**${client.language({ textId: `Достижение`, guildId: interaction.guildId, locale: interaction.locale })} <${achievement.name}> ${client.language({ textId: `было удалено`, guildId: interaction.guildId, locale: interaction.locale })}**`, flags: ["Ephemeral"] })
            } else
            if (args?.Subcommand === "view" || interaction.customId?.includes("view")) {
                ...
                const embed = new EmbedBuilder()
                    ...
                    }).slice(min, max).join("\n") : `${client.language({ textId: `Нет достижений`, guildId: interaction.guildId, locale: interaction.locale })}`)
                const embeds = [
                    embed,
                    new EmbedBuilder()
                        .setColor(3093046)
            ...
            if (args?.Subcommand === "enable-all") {
                if (!client.cache.achievements.some(e => e.guildID === interaction.guildId)) return interaction.editReply({ content: `${client.config.emojis.NO}**${client.language({ textId: `На сервере нет достижений`, guildId: interaction.guildId, locale: interaction.locale })}**`, flags: ["Ephemeral"] })
                let count = 0
                ...
                return interaction.editReply({ content: `${client.config.emojis.YES}**${client.language({ textId: `Достижения были включены`, guildId: interaction.guildId, locale: interaction.locale })} (${count})**`, flags: ["Ephemeral"] })
            } else
            if (args?.Subcommand === "disable-all") {
                ...
                return interaction.editReply({ content: `${client.config.emojis.YES}**${client.language({ textId: `Достижения были выключены`, guildId: interaction.guildId, locale: interaction.locale })} (${count})**`, flags: ["Ephemeral"] })
            }
        }
        if (!interaction.isChatInputCommand()) {
            achievement = client.cache.achievements.get(achievementRegexp.exec(interaction.customId)[1])
            if (!achievement) return interaction.reply({ content: `${client.config.emojis.NO}**${client.language({ textId: `Такого достижения не существует`, guildId: interaction.guildId, locale: interaction.locale })}**`, flags: ["Ephemeral"] })
            if (interaction.values[0] === "name") {
                const modal = new ModalBuilder()
                    .setCustomId(`manager-achievements_name_${interaction.id}`)
                    .setTitle(`${client.language({ textId: `Название достижения`, guildId: interaction.guildId, locale: interaction.locale })}`)
                    .setLabelComponents([
                        new LabelBuilder()
            ...
            if (interaction.values[0] === "type") {
                ...
                options.unshift({
                    label: `${client.language({ textId: `Своя задача`, guildId: interaction.guildId, locale: interaction.locale })}`,
                    value: "0"
                })
                const components = []
                const perChunk = 25
                        ...
                        new ActionRowBuilder()
                            .addComponents(
                                new StringSelectMenuBuilder()
                                    ...
                                    .setOptions(chunk)
                                    .setPlaceholder(`${client.language({ textId: `Задачи`, guildId: interaction.guildId, locale: interaction.locale })} (${chunk.length})`)
                            )
                    )
                })
                ...
                if (selectInteraction) {
                    ...
                    if (type === AchievementType.Craft || type === AchievementType.Item || type === AchievementType.MiningItem || type === AchievementType.FishingItem) {
                        const modal = new ModalBuilder()
                            ...
                            .setTitle(`${client.language({ textId: `Предметы (ИЛИ)`, guildId: selectInteraction.guildId, locale: selectInteraction.locale })}`)
                            .setLabelComponents([1, 2, 3, 4, 5].map(num => {
                                return new LabelBuilder()
                                    .setLabel(`${client.language({ textId: `Предмет`, guildId: selectInteraction.guildId, locale: selectInteraction.locale })} ${num}`)
                                    .setTextInputComponent(
                                        new TextInputBuilder()
                                            .setCustomId(`item${num}`)
                                            .setRequired(num === 1 ? true : false)
                                            .setStyle(TextInputStyle.Short)
                                            .setPlaceholder(`${client.language({ textId: `Название предмета`, guildId: selectInteraction.guildId, locale: selectInteraction.locale })}`)
                                    )
                            }))
                        ...
                        if (selectInteraction && selectInteraction.isModalSubmit()) {
                            ...
                            for (const itemName of values) {
                                ...
                                if (!item) {
                                    ...
                                    await selectInteraction.followUp({ content: `${client.config.emojis.NO}**${client.language({ textId: `Предмет с названием`, guildId: selectInteraction.guildId, locale: selectInteraction.locale })} <${itemName}> ${client.language({ textId: `не создан, либо он невидимый`, guildId: selectInteraction.guildId, locale: selectInteraction.locale })}**`, flags: ["Ephemeral"] })
                                } else {
                        ...
                        if ((type === AchievementType.Items && client.cache.achievements.some(e => e.type === AchievementType.Items && e.guildID === selectInteraction.guildId)) || (type === AchievementType.GetAllAchievements && client.cache.achievements.some(e => e.type === AchievementType.GetAllAchievements && e.guildID === selectInteraction.guildId))) {
                            ...
                            await selectInteraction.followUp({ content: `${client.config.emojis.NO}**${client.language({ textId: `Достижение с такой задачей уже существует`, guildId: selectInteraction.guildId, locale: selectInteraction.locale })}**`, flags: ["Ephemeral"] })
                        } else {
                        ...
                        const modal = new ModalBuilder()
                            ...
                            .setTitle(`${client.language({ textId: `Своя задача`, guildId: selectInteraction.guildId, locale: selectInteraction.locale })}`)
                            .setLabelComponents([
                                new LabelBuilder()
                                    .setLabel(`${client.language({ textId: `Задача`, guildId: selectInteraction.guildId, locale: selectInteraction.locale })}`)
                                    .setTextInputComponent(
                                        new TextInputBuilder()
                                            .setCustomId(`task`)
                                            .setRequired(true)
                        ...
                        const modal = new ModalBuilder()
                            ...
                            .setTitle(`${client.language({ textId: `Количество`, guildId: selectInteraction.guildId, locale: selectInteraction.locale })}`)
                            .setLabelComponents([
                                new LabelBuilder()
                                    .setLabel(`${client.language({ textId: `Количество`, guildId: selectInteraction.guildId, locale: selectInteraction.locale })}`)
                                    .setTextInputComponent(
                                        new TextInputBuilder()
                                            .setCustomId(`amount`)
                                            .setRequired(true)
                        ...
                        if (selectInteraction && selectInteraction.isModalSubmit()) {
                            ...
                            if (isNaN(+modalArgs.amount)) {
                                await selectInteraction.deferUpdate()
                                await selectInteraction.followUp({ content: `${client.config.emojis.NO}**${modalArgs.amount}${client.language({ textId: `не является числом`, guildId: interaction.guildId, locale: interaction.locale })}**`, flags: ["Ephemeral"] })
                            } else {
                                if (achievement.items.length) achievement.items = []
                                if (achievement.roles.length) achievement.roles = []
                                achievement.type = type
            ...
            if (interaction.values[0] === "addReward") {
                if (achievement.rewards.length >= MAX_REWARDS) return interaction.reply({ content: `${client.config.emojis.NO}**${client.language({ textId: `Достигнуто максимальное количество наград`, guildId: interaction.guildId, locale: interaction.locale })}: ${MAX_REWARDS}**`, flags: ["Ephemeral"] })
                    ...
                    if (interaction2.customId.includes("item")) {
                        ...
                        if (interaction2 && interaction2.isModalSubmit()) {
                            ...
                            if (!item) {
                                ...
                                await interaction2.followUp({ content: `${client.config.emojis.NO}**${client.language({ textId: `Предмет с названием`, guildId: interaction2.guildId, locale: interaction2.locale })} ${modalArgs.itemName} ${client.language({ textId: `не создан, либо он невидимый`, guildId: interaction2.guildId, locale: interaction2.locale })}**`, flags: ["Ephemeral"] })
                            } else {
                                if (isNaN(+modalArgs.amount)) {
                                    await interaction2.deferUpdate()
                                    await interaction2.followUp({ content: `${client.config.emojis.NO}**${modalArgs.amount}${client.language({ textId: `не является числом`, guildId: interaction2.guildId, locale: interaction2.locale })}**`, flags: ["Ephemeral"] })
                                } else {
                                    ...
                                    if (modalArgs.amount > max || modalArgs.amount < min) {
                                        await interaction2.deferUpdate()
                                        await interaction2.followUp({ content: `${client.config.emojis.NO}**${client.language({ textId: `Количество не должно быть`, guildId: interaction.guildId, locale: interaction2.locale })} > ${max} ${client.language({ textId: `или`, guildId: interaction2.guildId, locale: interaction2.locale })} < ${min}**`, flags: ["Ephemeral"] })
                                    } else {
                        ...
                        if (interaction2) {
                            ...
                            if (reward) {
                                await interaction2.deferUpdate()
                                await interaction2.followUp({ content: `${client.config.emojis.NO}**${client.language({ textId: `Такая роль в наградах уже есть`, guildId: interaction2.guildId, locale: interaction2.locale })}**`, flags: ["Ephemeral"] })
                            } else {
                                achievement.rewards.push({
                        ...
                        if (interaction2 && interaction2.isModalSubmit()) {
                            ...
                            if (isNaN(+modalArgs.amount)) {
                                ...
                                if (modalArgs.amount > max || modalArgs.amount < min) {
                                    await interaction2.deferUpdate()
                                    await interaction2.followUp({ content: `${client.config.emojis.NO}**${client.language({ textId: `Количество не должно быть`, guildId: interaction2.guildId, locale: interaction2.locale })} > ${max} ${client.language({ textId: `или`, guildId: interaction2.guildId, locale: interaction2.locale })} < ${min}**`, flags: ["Ephemeral"] })
                                } else {
            ...
            if (interaction.values[0] === "delReward") {
                if (!achievement.rewards.length) return interaction.reply({ content: `${client.config.emojis.NO}**${client.language({ textId: `У достижения нет наград`, guildId: interaction.guildId, locale: interaction.locale })}**`, flags: ["Ephemeral"] })
            ...
            if (interaction.values[0] === "switch") {
                const errors = []
                if (!achievement.emoji) errors.push(`${client.language({ textId: `Отсутствует эмодзи`, guildId: interaction.guildId, locale: interaction.locale })}`)
                if (!achievement.type) errors.push(`${client.language({ textId: `Отсутствует задача`, guildId: interaction.guildId, locale: interaction.locale })}`)
                if (errors.length) return interaction.reply({ content: `${client.config.emojis.NO}**${client.language({ textId: `Невозможно включить достижение`, guildId: interaction.guildId, locale: interaction.locale })}:**\n* ${errors.join("\n* ")}`, flags: ["Ephemeral"] })
            ...
            if (interaction.values[0] === "addAllUsers") {
                ...
                await interaction.followUp({ content: `${client.config.emojis.YES}**${client.language({ textId: `Достижение было выдано пользователям`, guildId: interaction.guildId, locale: interaction.locale })} (${count})**`, flags: ["Ephemeral"] })
            } else
            if (interaction.values[0] === "removeAllUsers") {
                ...
                await interaction.followUp({ content: `${client.config.emojis.YES}**${client.language({ textId: `Достижение было убрано у пользователей`, guildId: interaction.guildId, locale: interaction.locale })} (${count})**`, flags: ["Ephemeral"] })
            }
        }
        const embed = new EmbedBuilder()
            .setAuthor({ name: `${client.language({ textId: `Менеджер достижений`, guildId: interaction.guildId, locale: interaction.locale })}` })
            ...
            .setFields([
                {
                    name: client.language({ textId: `Награды`, guildId: interaction.guildId, locale: interaction.locale }),
                    value: achievement.rewards.length ? await Promise.all(achievement.rewards.map(async reward => {
                        ...
                        const name = 
                            ...
                            reward.type === RewardType.Experience ? client.language({ textId: `Опыт`, guildId: interaction.guildId, locale: interaction.locale }) : 
                            reward.type === RewardType.Reputation ? client.language({ textId: `Репутация`, guildId: interaction.guildId, locale: interaction.locale }) : 
                    ...
                    })).then(array => array.join(", ")) : client.language({ textId: `Нет наград`, guildId: interaction.guildId, locale: interaction.locale })
                }
            ])
            .setColor(3093046)
            ...
            .addComponents(
                new StringSelectMenuBuilder()
                    .setOptions([
                        ...
                        {
                            label: client.language({ textId: `Изменить задачу`, guildId: interaction.guildId, locale: interaction.locale }),
                            value: `type`,
                            emoji: "📋"
                        },
                        {
                    ...
                    .setPlaceholder(`✏️${client.language({ textId: `Редактировать достижение`, guildId: interaction.guildId, locale: interaction.locale })}`)
        ...
        const row2 = new ActionRowBuilder()
            .addComponents(
                new StringSelectMenuBuilder()
                    .setOptions([
                        {
                            label: achievement.enabled ? client.language({ textId: `Выключить достижение`, guildId: interaction.guildId, locale: interaction.locale }) : client.language({ textId: `Включить достижение`, guildId: interaction.guildId, locale: interaction.locale }),
                            value: `switch`,
