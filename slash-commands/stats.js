    ...
    run: async (client, interaction, args) => {
        ...
        if (filter === "alltime") {
            description.push(
                ...
                `${client.language({ textId: `Уровень`, guildId: interaction.guildId, locale: interaction.locale })}: ${client.config.emojis.medal} ${profile.level}`,
                `${client.language({ textId: `Опыт`, guildId: interaction.guildId, locale: interaction.locale })}: ${client.config.emojis.XP} ${profile.totalxp.toFixed()}`,
                settings.seasonLevelsEnabled ? `${client.language({ textId: `Сезонный опыт`, guildId: interaction.guildId, locale: interaction.locale })}: ${client.config.emojis.seasonXP} ${profile.seasonTotalXp.toFixed()}` : false,
                ...
                `* ${client.language({ textId: `продано на маркете`, guildId: interaction.guildId, locale: interaction.locale })}: ${profile.itemsSoldOnMarketPlace}`,
                `* ${client.language({ textId: `открыто`, guildId: interaction.guildId, locale: interaction.locale })}: ${profile.itemsOpened}`,
                `* ${client.language({ textId: `получено`, guildId: interaction.guildId, locale: interaction.locale })}: ${profile.itemsReceived}`,
                `* ${client.language({ textId: `скрафчено`, guildId: interaction.guildId, locale: interaction.locale })}: ${profile.itemsCrafted}`,
                `* ${client.language({ textId: `использовано`, guildId: interaction.guildId, locale: interaction.locale })}: ${profile.itemsUsed}`,
                `* ${client.language({ textId: `куплено в магазине`, guildId: interaction.guildId, locale: interaction.locale })}: ${profile.itemsBoughtInShop}`,
                `* ${client.language({ textId: `куплено на маркете`, guildId: interaction.guildId, locale: interaction.locale })}: ${profile.itemsBoughtOnMarket}`,
                `* ${client.language({ textId: `продано`, guildId: interaction.guildId, locale: interaction.locale })}: ${profile.itemsSold}`,
        )
        } else {
            description.push(
                `# ${client.language({ textId: `Статистика за ${filter}`, guildId: interaction.guildId, locale: interaction.locale })}:`,
                `${client.language({ textId: `Опыт`, guildId: interaction.guildId, locale: interaction.locale })}: ${client.config.emojis.XP} ${profile.stats?.[filter]?.totalxp?.toFixed() || 0}`,
                ...
                `${client.language({ textId: `Сообщения`, guildId: interaction.guildId, locale: interaction.locale })}: ${client.config.emojis.message} ${profile.stats?.[filter]?.messages || 0}`,
                `${client.language({ textId: `Лайки`, guildId: interaction.guildId, locale: interaction.locale })}: ${client.config.emojis.heart} ${profile.stats?.[filter]?.likes || 0}`,
                `${settings.currencyName}: ${settings.displayCurrencyEmoji} ${profile.stats?.[filter]?.currency?.toFixed() || 0}`,
                `${client.language({ textId: `Приглашения`, guildId: interaction.guildId, locale: interaction.locale })}: ${client.config.emojis.invite} ${profile.stats?.[filter]?.invites || 0}`,
                `${client.language({ textId: `Бампы`, guildId: interaction.guildId, locale: interaction.locale })}: ${client.config.emojis.bump} ${profile.stats?.[filter]?.bumps || 0}`,
                `${client.language({ textId: `Репутация`, guildId: interaction.guildId, locale: interaction.locale })}: ${client.config.emojis.RP} ${profile.stats?.[filter]?.rp?.toFixed(2) || 0}`,
                `${client.language({ textId: `Квестов завершено`, guildId: interaction.guildId, locale: interaction.locale })}: ${client.config.emojis.quests} ${profile.stats?.[filter]?.doneQuests || 0}`,
                `${client.language({ textId: `Раздачи`, guildId: interaction.guildId, locale: interaction.locale })}: ${client.config.emojis.giveaway} ${profile.stats?.[filter]?.giveawaysCreated || 0}`,
                `${client.language({ textId: `Червоточины`, guildId: interaction.guildId, locale: interaction.locale })}: ${client.config.emojis.wormhole} ${profile.stats?.[filter]?.wormholeTouched || 0}`,
                `${client.language({ textId: `Предметов продано на маркете`, guildId: interaction.guildId, locale: interaction.locale })}: ${client.config.emojis.box} ${profile.stats?.[filter]?.itemsSoldOnMarketPlace || 0}`,
            )
        }
        embed.setDescription(description.filter(e => e).join("\n"))
        embed.setColor(member.displayHexColor)
        embed.setThumbnail(member.displayAvatarURL())
        embed.setFooter({ text: `${client.language({ textId: `До следующего уровня`, guildId: interaction.guildId, locale: interaction.locale })} ⭐${profile.xp.toFixed()}/${profile.level * settings.levelfactor + 100} XP (${Math.floor((profile.xp / (profile.level * settings.levelfactor + 100)) * 100)}%)${settings.seasonLevelsEnabled ? `\n${client.language({ textId: `До следующего сезонного уровня`, guildId: interaction.guildId, locale: interaction.locale })} ⭐${profile.seasonXp.toFixed()}/${profile.seasonLevel * settings.seasonLevelfactor + 100} XP (${Math.floor((profile.seasonXp / (profile.seasonLevel * settings.seasonLevelfactor + 100)) * 100)}%)` : ""}`, iconURL: client.user.displayAvatarURL() })
